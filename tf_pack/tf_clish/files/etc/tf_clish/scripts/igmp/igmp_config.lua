...
local function igmp_config_Max_snooping_table_size(option)
    ...
    --    io.write("error: IGMP in disable state", "\n")
...
local function igmp_config_Robustness(option)
    ...
    --    io.write("error: multicast querier in disable state - not defined", "\n")
    --elseif multicastQuerierStatus == igmp.errorCode.zero then
    --    io.write("error: multicast querier in disable state - 0", "\n")
    --else
...
local function igmp_config_Query_response_interval(option)
    ...
    if queryIntervalValue == igmp.errorCode.notdef then
        io.write("error: Query Response Interval must be set after Query Interval", "\n")
    elseif queryIntervalValue == igmp.errorCode.zero then
        io.write("error: Query Response Interval must be lower than the query interval value", "\n")
    elseif responseIntervalValue >= (tonumber(queryIntervalValue)/canti_sec) then
        io.write("error: the Query Response Interval must be lower than the query interval value: ", tostring(tonumber(queryIntervalValue)/canti_sec), "\n")
    else
...
local function igmp_config_Last_member_interval(option)
    ...
    --    io.write("error: multicast querier in disable state", "\n")
    --else
...
local function config_igmp()
    local error = 1
    for index, value in pairs(igmp_cmdList) do
