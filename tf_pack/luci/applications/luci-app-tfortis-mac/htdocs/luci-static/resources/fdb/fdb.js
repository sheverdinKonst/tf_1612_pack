...
function logviewTableAddRow(plugin, table, data, columns, filterPattern) {
	...
	for (var i = 0; i < columns.length; i++) {
		...
		if ((name == 'vid') && data.hasOwnProperty(key)) {
			vid = columns[i].modfunc(data[key]).trim();
		}
		...
		if (data.hasOwnProperty(key)) {
			if (name == 'timestamp') {
				...
				cell_data = priorityDisplay.hasOwnProperty(vid)
					? priorityDisplay[vid] : vid;
			}
			else {
	...
	r.classList.add('lv-p-' + vid);

	if (!filterMatch)
		return 0;

	/* Checking for vid visibility and update vids count */
	if (vid !== 'none') { /* line.hasOwnProperty('vid') */
		if (plugin.vids.hasOwnProperty(vid))
			plugin.vids[vid].count++;
		else {
			plugin.vids[vid] = {
				'count': 1,
				'display': priorityDisplay.hasOwnProperty(vid)
					? priorityDisplay[vid]
					: vid
			};
		}

		if (plugin.vids[vid].hide)
			return 0;
	}
...
function logviewTableUpdate(plugin, logData, filterPattern) {
	...
	/* Clear all vids count */
	Object.keys(plugin.vids).forEach(function(vid) {
		plugin.vids[vid].count = 0;
	});
		...
		logData.forEach(function(line) {
			for (let i = 1; i < plugin.columns.length; i++) {
				if (!plugin.columns[i].empty)
					continue;
				if(line['interface'])
					line['interface'] = getPortName(line['interface']);
...
return L.Class.extend({
	load: function() {
		return Promise.all([
			L.resolveDefault(fs.list('/www' + L.resource('fdb/plugins')), [])
			...
			for (var i = 0; i < plugins.length; i++) {
				...
				tasks.push(L.require('fdb.plugins.' + m[1]).then(L.bind(function(name, plugin) {
					if (!plugin.acl)
						return;
					...
					return callSessionAccess('access-group', plugin.acl, 'read').then(L.bind(function(name, plugin, access) {
						...
						plugin.opts = {};
						plugin.name = name;
						plugin.opts.sortDescending = false;
	...
	logPriorities: function(logName) {
		...
		return logviewPlugins[logName].vids;
	},

	setVlanVisible: function(logName, vid, visible) {
		if (!logviewPlugins[logName])
			return;

		var plugin = logviewPlugins[logName];
		if (plugin.vids.hasOwnProperty(vid))
			plugin.vids[vid].hide = !visible;
	},
