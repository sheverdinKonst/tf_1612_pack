...
var callPortStatistics = rpc.declare({
	object: 'luci-rpc',
...
function stat(ifame,param){
	return port_statistics[0][ifame].stats[param];
}
...
return view.extend({
	...
	pollData: function() {
		poll.add(L.bind(function() {
			...
			tasks.push(L.resolveDefault(callPortStatistics(), []));

			return Promise.all(tasks).then(L.bind(function(datasets) {

				port_statistics = datasets.slice(-1);
	...
	render: function(data) {
		...
		for (var i = 0; i < devs.length; i++) {
			...
			v.firstElementChild.appendChild(E('div', { 'data-tab': ifname, 'data-tab-title': getPortName(ifname) }, [
				csvg,
				...
				E('table', { 'class': 'table', 'style': 'width:100%;table-layout:fixed' }, [
					...
					E('tr', { 'class': 'tr' }, [
						E('td', { 'class': 'td right top' }, E('strong', [ _('RX bytes:') ])),
						E('td', { 'class': 'td', 'id': 'rx_bytes' }, 0),

						E('td', { 'class': 'td right top' }, E('strong', [ _('TX bytes:') ])),
						E('td', { 'class': 'td', 'id': 'tx_bytes' }, 0),
					]),


					E('tr', { 'class': 'tr' }, [
						E('td', { 'class': 'td right top' }, E('strong',  [ _('RX packets:') ])),
						E('td', { 'class': 'td', 'id': 'rx_packets' }, 0),

						E('td', { 'class': 'td right top' }, E('strong', [ _('TX packets:') ])),
						E('td', { 'class': 'td', 'id': 'tx_packets' }, 0),
					]),

					E('tr', { 'class': 'tr' }, [
						E('td', { 'class': 'td right top' }, E('strong', [ _('RX errors:') ])),
						E('td', { 'class': 'td', 'id': 'rx_errors' }, 0),

						E('td', { 'class': 'td right top' }, E('strong', [ _('TX errors:') ])),
						E('td', { 'class': 'td', 'id': 'tx_errors' }, 0),
					]),

					E('tr', { 'class': 'tr' }, [
						E('td', { 'class': 'td right top' }, E('strong', [ _('RX dropped:') ])),
						E('td', { 'class': 'td', 'id': 'rx_dropped' }, 0),

						E('td', { 'class': 'td right top' }, E('strong', [ _('TX dropped:') ])),
						E('td', { 'class': 'td', 'id': 'tx_dropped' }, 0),
					]),

					E('tr', { 'class': 'tr' }, [
						E('td', { 'class': 'td right top' }, E('strong', [ _('RX multicast:') ])),
						E('td', { 'class': 'td', 'id': 'rx_multicast' }, 0),

						E('td', { 'class': 'td right top' }, E('strong', [ _('TX collisions:') ])),
						E('td', { 'class': 'td', 'id': 'tx_collisions' }, 0),
					])
				])
			]));
			...
			this.updateGraph(ifname, csvg, [ { line: 'rx', counter: true }, null, { line: 'tx', counter: true } ], function(svg, info) {
				...
				dom.content(tab.querySelector('#tx_bw_peak1'), rate(info.line_peak[2], true));

				//detail
				dom.content(tab.querySelector('#rx_bytes'), stat(info.ifname,'rx_bytes'));
				dom.content(tab.querySelector('#rx_packets'), stat(info.ifname,'rx_packets'));
				dom.content(tab.querySelector('#multicast'), stat(info.ifname,'multicast'));
				dom.content(tab.querySelector('#rx_errors'), stat(info.ifname,'rx_errors'));
				dom.content(tab.querySelector('#rx_dropped'), stat(info.ifname,'rx_dropped'));

				dom.content(tab.querySelector('#tx_bytes'), stat(info.ifname,'tx_bytes'));
				dom.content(tab.querySelector('#tx_packets'), stat(info.ifname,'tx_packets'));
				dom.content(tab.querySelector('#tx_errors'), stat(info.ifname,'tx_errors'));
				dom.content(tab.querySelector('#collisions'), stat(info.ifname,'collisions'));
				dom.content(tab.querySelector('#tx_dropped'), stat(info.ifname,'tx_dropped'));
